...
Data Sources:
    - kindergarten_readiness_analysis_{student_group}.csv: Merged dataset with school-level reading rates
...
INDICATOR_ID = 'kindergarten_readiness'
INDICATOR_NAME = 'Kindergarten Readiness Rate'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'kindergarten_readiness_rate'
ANALYSIS_FILE_BASE = 'kindergarten_readiness_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    ...
    Args:
        data_path: Path to kindergarten_readiness_analysis.csv
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level kindergarten readiness rate statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average kindergarten readiness rates.

    This measures how much districts differ from each other in their
    typical kindergarten readiness rates.
    """
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in kindergarten readiness rates,
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR KINDERGARTEN READINESS RATE PRIOR SPECIFICATION")
    ...
    state_stats = compute_state_level_statistics(df)
    print(f"\nState kindergarten readiness rate:")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level kindergarten readiness rates (n={district_stats['n_districts']} districts):")
