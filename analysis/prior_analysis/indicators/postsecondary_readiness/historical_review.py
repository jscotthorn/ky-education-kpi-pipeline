#!/usr/bin/env python3
"""
Historical Data Review for Third Grade Reading Prior Specification

This script implements Stage 1 (Empirical Foundation) of the hybrid approach
outlined in PRIOR_SPECIFICATION_GUIDE.md. It analyzes historical third grade
reading proficiency data from Kentucky schools to establish baseline variance
estimates for Bayesian hierarchical model priors.

Data Sources:
    - postsecondary_readiness_analysis_{student_group}.csv: Merged dataset with school-level reading rates
...
INDICATOR_ID = 'postsecondary_readiness'
INDICATOR_NAME = 'Postsecondary Readiness Rate'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'postsecondary_readiness_rate'
ANALYSIS_FILE_BASE = 'postsecondary_readiness_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    """
    Load and filter reading analysis data for specified years.

    Args:
        data_path: Path to postsecondary_readiness_analysis.csv
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level postsecondary readiness rate statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average postsecondary readiness rates.

    This measures how much districts differ from each other in their
    typical postsecondary readiness rates.
    """
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in postsecondary readiness rates,
    ...
    return {
        ...
        'pct_large_changes': ((pivot['change'].abs() > 10).sum() / len(pivot)) * 100  # 10pp threshold for reading
    }
...
def recommend_priors(components: VarianceComponents) -> Dict[str, Dict[str, Any]]:
    ...
    return {
        'mu_state': {
            ...
            'scale': 5.0,  # Wider scale for reading (more variation than graduation)
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR POSTSECONDARY READINESS RATE PRIOR SPECIFICATION")
    ...
    state_stats = compute_state_level_statistics(df)
    print(f"\nState postsecondary readiness rate:")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level postsecondary readiness rates (n={district_stats['n_districts']} districts):")
...
2. PRIOR PREDICTIVE CHECK: Simulate from proposed priors to verify
   - 95% of simulated rates should fall in [~20%, ~70%]
