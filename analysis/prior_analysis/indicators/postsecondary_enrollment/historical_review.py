#!/usr/bin/env python3
"""
Historical Data Review for Postsecondary Enrollment Prior Specification

This script implements Stage 1 (Empirical Foundation) of the hybrid approach
outlined in PRIOR_SPECIFICATION_GUIDE.md. It analyzes historical postsecondary
enrollment data from Kentucky schools to establish baseline variance
estimates for Bayesian hierarchical model priors.

Data Sources:
    - postsecondary_enrollment_analysis_{student_group}.csv: Merged dataset with school-level enrollment rates
...
INDICATOR_ID = 'postsecondary_enrollment'
INDICATOR_NAME = 'Postsecondary Enrollment Rate'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'postsecondary_enrollment_rate'
ANALYSIS_FILE_BASE = 'postsecondary_enrollment_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    """
    Load and filter enrollment analysis data for specified years.

    Args:
        data_path: Path to postsecondary_enrollment_analysis.csv
    ...
    assert RATE_COLUMN in df_filtered.columns, f"Missing {RATE_COLUMN} column"
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level postsecondary enrollment rate statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average postsecondary enrollment rates.

    This measures how much districts differ from each other in their
    typical postsecondary enrollment rates.
    """
    district_means = df.groupby('district')[RATE_COLUMN].mean()
...
def compute_within_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    school_means = df.groupby(['district', 'school_id'])[RATE_COLUMN].mean().reset_index()
    ...
    if len(multi_school_districts) > 0:
        within_sd_by_district = multi_school_districts.groupby('district')[RATE_COLUMN].std()
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in postsecondary enrollment rates,
    ...
    pivot = schools_both_years.pivot(index='school_id', columns='year', values=RATE_COLUMN)
    ...
    return {
        ...
        'pct_large_changes': ((pivot['change'].abs() > 10).sum() / len(pivot)) * 100  # 10pp threshold
    }
...
def recommend_priors(components: VarianceComponents) -> Dict[str, Dict[str, Any]]:
    """
    Translate variance components into recommended prior parameters.

    Following guidance from PRIOR_SPECIFICATION_GUIDE.md:
    - Use HalfNormal with scale = 1.5x empirical SD for variance parameters
    ...
    return {
        'mu_state': {
            ...
            'scale': 5.0,  # Wider scale for enrollment (more variation)
            'rationale': f'Empirical mean {components.state_mean:.1f}%, scale gives 95% CI of ~10 points',
            '95_interval': [float(round(max(0, components.state_mean - 10), 1)), float(round(min(100, components.state_mean + 10), 1))]
        },
        'sigma_district': {
            'distribution': 'HalfNormal',
            'scale': round(components.between_district_sd * 1.5, 1),
            'empirical_sd': round(components.between_district_sd, 2),
            'rationale': 'Scale = 1.5x empirical between-district SD'
        },
        'sigma_school': {
            'distribution': 'HalfNormal',
            'scale': round(components.within_district_sd * 1.5, 1),
            'empirical_sd': round(components.within_district_sd, 2),
            'rationale': 'Scale = 1.5x empirical within-district school deviation SD'
        },
        'sigma_y': {
            'distribution': 'HalfNormal',
            'scale': round(components.year_to_year_sd * 1.5, 1),
            'empirical_sd': round(components.year_to_year_sd, 2),
            'rationale': 'Scale = 1.5x implied observation SD from year-to-year changes'
        }
    }
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR POSTSECONDARY ENROLLMENT RATE PRIOR SPECIFICATION")
    ...
    state_stats = compute_state_level_statistics(df)
    print(f"\nState postsecondary enrollment rate:")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level postsecondary enrollment rates (n={district_stats['n_districts']} districts):")
    ...
    print(f"  (computed as SD(changes) / sqrt(2))")
    ...
    if temporal_stats['change_percentiles']:
        ...
        if not np.isnan(pct_large):
            print(f"\nSchools with |change| > 10pp: {pct_large:.1f}%")
...
def run_for_group(student_group: str) -> dict:
    """Run prior analysis for a single student group."""
    script_dir = Path(__file__).parent
    prior_analysis_dir = script_dir.parent.parent
    project_root = prior_analysis_dir.parent.parent

    # Data path: {indicator}_analysis_{student_group}.csv
    ...
    priors = recommend_priors(components)

    print_report(df, components, priors)
    save_results(components, priors, output_dir, student_group)

    return {'components': components, 'priors': priors}


def main():
    """Main execution function."""
    parser = argparse.ArgumentParser(description=f"Run prior analysis for {INDICATOR_NAME}")
...
2. PRIOR PREDICTIVE CHECK: Simulate from proposed priors to verify
   - 95% of simulated rates should fall in reasonable range
   - No predictions below 0% or exceeding 100%
