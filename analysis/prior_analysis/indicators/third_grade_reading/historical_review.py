...
Data Sources:
    - reading_grade3_analysis_{student_group}.csv: Merged dataset with school-level reading rates
...
INDICATOR_ID = 'third_grade_reading'
INDICATOR_NAME = 'Third Grade Reading Proficiency'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'reading_proficiency_rate'
ANALYSIS_FILE_BASE = 'reading_grade3_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    ...
    Args:
        data_path: Path to reading_grade3_analysis.csv
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level reading proficiency rate statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average reading proficiency rates.

    This measures how much districts differ from each other in their
    typical reading proficiency rates.
    """
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in reading proficiency rates,
...
def recommend_priors(components: VarianceComponents, multiplier: float = 1.5) -> Dict[str, Dict[str, Any]]:
    ...
    state_mean_scale = max(2.0, 5.0 * (multiplier / 1.5))
    ...
    return {
        'mu_state': {
            ...
            '95_interval': [float(round(max(0, components.state_mean - 2*state_mean_scale), 1)), float(round(min(100, components.state_mean + 2*state_mean_scale), 1))]
        },
        'sigma_district': {
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR THIRD GRADE READING PRIOR SPECIFICATION")
    ...
    state_stats = compute_state_level_statistics(df)
    print(f"\nState reading proficiency rate:")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level reading proficiency rates (n={district_stats['n_districts']} districts):")
