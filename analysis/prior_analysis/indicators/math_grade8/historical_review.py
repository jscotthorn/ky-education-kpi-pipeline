...
Data Sources:
    - math_grade8_analysis_{student_group}.csv: Merged dataset with school-level reading rates
...
INDICATOR_ID = 'math_grade8'
INDICATOR_NAME = '8th Grade Math Proficiency'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'math_proficiency_rate'
ANALYSIS_FILE_BASE = 'math_grade8_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    ...
    Args:
        data_path: Path to math_grade8_analysis.csv
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level 8th grade math proficiency statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average 8th grade math proficiencys.

    This measures how much districts differ from each other in their
    typical 8th grade math proficiencys.
    """
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in 8th grade math proficiencys,
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR 8TH GRADE MATH PRIOR SPECIFICATION")
    ...
    state_stats = compute_state_level_statistics(df)
    print(f"\nState 8th grade math proficiency:")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level 8th grade math proficiencys (n={district_stats['n_districts']} districts):")
