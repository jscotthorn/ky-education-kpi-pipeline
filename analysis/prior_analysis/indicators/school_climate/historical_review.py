...
Data Sources:
    - school_climate_analysis_{student_group}.csv: Merged dataset with school-level reading rates
...
INDICATOR_ID = 'school_climate'
INDICATOR_NAME = 'School Climate Index'
ANALYSIS_YEARS = [2023, 2024]  # year=2023 is 2022-23, year=2024 is 2023-24
RATE_COLUMN = 'school_climate_index'
ANALYSIS_FILE_BASE = 'school_climate_analysis'
...
def load_historical_data(data_path: Path, years: list[int]) -> pd.DataFrame:
    ...
    Args:
        data_path: Path to school_climate_analysis.csv
...
def compute_state_level_statistics(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute state-level school climate index statistics.
...
def compute_between_district_variation(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Compute variation between district average school climate indexs.

    This measures how much districts differ from each other in their
    typical school climate indexs.
    """
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    This estimates the observation-level noise in school climate indexs,
    ...
    return {
        ...
        'pct_large_changes': ((pivot['change'].abs() > 5).sum() / len(pivot)) * 100  # 5pp threshold
    }
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print("HISTORICAL DATA REVIEW FOR SCHOOL CLIMATE INDEX PRIOR SPECIFICATION")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level school climate indexs (n={district_stats['n_districts']} districts):")
