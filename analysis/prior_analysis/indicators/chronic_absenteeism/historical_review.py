#!/usr/bin/env python3
"""
Historical Data Review for Chronic Absenteeism Prior Specification

This script implements Stage 1 (Empirical Foundation) of the hybrid approach
outlined in PRIOR_SPECIFICATION_GUIDE.md. It analyzes historical chronic
absenteeism data from Kentucky schools to establish baseline variance estimates
for Bayesian hierarchical model priors.

Data Sources:
    - chronic_absenteeism_analysis_{student_group}.csv: Merged dataset with school-level rates
...
RATE_COLUMN = 'chronic_absenteeism_rate'
ANALYSIS_FILE_BASE = 'chronic_absenteeism_analysis'
...
def compute_year_to_year_variation(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    return {
        ...
        'pct_large_changes': ((pivot['change'].abs() > 10).sum() / len(pivot)) * 100
    }
...
def print_report(df: pd.DataFrame, components: VarianceComponents, priors: Dict) -> None:
    ...
    print("=" * 80)
    print(f"HISTORICAL DATA REVIEW FOR {INDICATOR_NAME.upper()} PRIOR SPECIFICATION")
    ...
    district_stats = compute_between_district_variation(df)
    print(f"\nDistrict-level rates (n={district_stats['n_districts']} districts):")
...
EMPIRICAL ESTIMATES:
--------------------
1. State Mean (mu_state): {components.state_mean:.1f}%
2. Between-District SD (sigma_district): {components.between_district_sd:.2f}%
3. Within-District/School SD (sigma_school): {components.within_district_sd:.2f}%
4. Observation Noise SD (sigma_y): {components.year_to_year_sd:.2f}%
""")
