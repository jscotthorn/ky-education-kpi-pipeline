...
Output: analysis/datasets/el_progress_middle_analysis.csv
"""
...
class ELProgressMiddleAnalysisDataset(BaseAnalysisDataset):
    ...
    def load_outcome_data(self) -> pd.DataFrame:
        ...
        # Filter to EL progress score 140 (proficiency) for middle
        el_df = df[df['metric'] == 'english_learner_score_140_middle'].copy()
...
def main():
    """Main execution."""
    print("=" * 60)
    print("CREATE EL PROGRESS MIDDLE ANALYSIS DATASET")
    print("=" * 60)

    dataset = ELProgressMiddleAnalysisDataset(verbose=True)
