#!/usr/bin/env python3
"""
English Learner Progress Analysis Dataset - Middle Schools
...
Output: analysis/datasets/el_progress_middle_analysis_{student_group}.csv

Usage:
    python el_progress_middle_analysis.py                           # All Students (default)
    python el_progress_middle_analysis.py --student-group african_american
    python el_progress_middle_analysis.py --all-groups              # Run for all target groups
"""
...
class ELProgressMiddleAnalysisDataset(BaseAnalysisDataset):
    """
    Analysis dataset for middle school EL progress rates.

    Filters:
    - metric == 'english_learner_score_140_middle'
    - All Students only
    - Unsuppressed values
    """

    OUTCOME_NAME = 'el_progress_rate'
    OUTPUT_FILENAME = 'el_progress_middle_analysis.csv'
    ...
    def load_outcome_data(self) -> pd.DataFrame:
        ...
        self.log("LOADING EL PROGRESS DATA (MIDDLE)", header=True)

        # Use chunked reading to efficiently load from large KPI file
        el_df = self.read_kpi_chunked(['english_learner_score_140_middle'])
        self.log(f"Found {len(el_df):,} EL progress middle records")
...
def run_for_group(student_group: str) -> pd.DataFrame:
    ...
    print("=" * 60)
    print(f"CREATE EL PROGRESS MIDDLE ANALYSIS DATASET")
    print(f"Student Group: {student_group}")
    print("=" * 60)

    dataset = ELProgressMiddleAnalysisDataset(verbose=True, student_group=student_group)
    df = dataset.create_dataset()

    print("\n" + "=" * 60)
    print("EL PROGRESS MIDDLE ANALYSIS DATASET COMPLETE")
    print("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create EL progress middle analysis dataset for specified student group"
    )
    parser.add_argument(
        '--student-group',
        type=str,
