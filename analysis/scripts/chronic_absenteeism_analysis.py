...
Output: analysis/datasets/chronic_absenteeism_analysis_{student_group}.csv

Usage:
    python chronic_absenteeism_analysis.py                           # All Students (default)
    python chronic_absenteeism_analysis.py --student-group african_american
    python chronic_absenteeism_analysis.py --all-groups              # Run for all target groups
"""
...
class ChronicAbsenteeismAnalysisDataset(BaseAnalysisDataset):
    ...
    def load_outcome_data(self) -> pd.DataFrame:
        ...
        ca_df = ca_df[ca_df['student_group'] == self.student_group_name].copy()
        self.log(f"{self.student_group_name}: {len(ca_df):,} records")
...
def run_for_group(student_group: str) -> pd.DataFrame:
    ...
    print("=" * 60)
    print(f"CREATE CHRONIC ABSENTEEISM ANALYSIS DATASET")
    print(f"Student Group: {student_group}")
    print("=" * 60)

    dataset = ChronicAbsenteeismAnalysisDataset(verbose=True, student_group=student_group)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create chronic absenteeism analysis dataset for specified student group"
    )
    parser.add_argument(
        '--student-group',
        type=str,
