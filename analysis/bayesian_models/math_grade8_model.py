...
Usage:
    ...
    python math_grade8_model.py --student-group african_american
    python math_grade8_model.py --all-groups
"""
...
class MathGrade8Model(BaseHierarchicalModel):
    ...
    def get_state_mean_prior(self) -> Tuple[float, float]:
        """KY 8th grade math proficiency ~37.2% (from empirical prior analysis)."""
        return (37.2, 5.0)

    def get_variance_priors(self) -> Dict[str, float]:
        """
        Variance priors from empirical analysis (1.5x observed SD).
        Math shows highest district variation among academic indicators.
        """
        return {
            'sigma_district': 18.7,
            'sigma_school': 15.0,
...
def run_for_group(student_group: str, prior_type: str, non_centered: bool,
    ...
    model = MathGrade8Model(verbose=True, student_group=student_group)
    return model.run(
