"""
End-to-end validation test for teacher turnover ETL pipeline.
...
class TestTeacherTurnoverEndToEnd:
    ...
    def test_source_to_kpi_transformation(self):
        ...
        raw_data_dir = Path("data/raw/teacher_turnover")
        processed_file = Path("data/processed/teacher_turnover.csv")

        # Ensure processed file exists
        if not processed_file.exists():
            pytest.skip("Processed teacher_turnover.csv not found. Run ETL pipeline first.")
    ...
    def _test_source_file_transformation(self, source_file: Path, kpi_df: pd.DataFrame):
        ...
        # Filter to rows with turnover data
        turnover_cols = [col for col in source_df.columns if 'turnover' in col.lower() or 'teacher count' in col.lower()]
        has_data = source_df[turnover_cols].notna().any(axis=1)
        source_df_with_data = source_df[has_data]

        if source_df_with_data.empty:
            pytest.skip(f"Source file {source_file.name} has no rows with turnover data")
    ...
    def test_all_expected_metrics_present(self):
        """Test that all turnover metrics are present in the processed data."""
        processed_file = Path("data/processed/teacher_turnover.csv")
        ...
        print(f"✓ All {len(expected_metrics)} expected turnover metrics found")
    ...
    def test_value_ranges(self):
        """Test that turnover values are within reasonable ranges."""
        processed_file = Path("data/processed/teacher_turnover.csv")
        ...
        rate_df = kpi_df[kpi_df['metric'] == 'teacher_turnover_rate']
        rate_values = rate_df['value'].dropna()

        # Rates should be non-negative
        assert (rate_values >= 0).all(), "Found negative turnover rates"

        # Note: Turnover rates CAN legitimately exceed 100% when positions turn over
        # multiple times within the measurement period (e.g., a position filled and
        # vacated 3 times = 300% turnover for that position). This is standard HR
        # methodology per BLS - hospitality industry regularly exceeds 100%.
        # Flag extreme values (>200%) for awareness but don't exclude them.
        extreme_outliers = (rate_values > 200).sum()
        if extreme_outliers > 0:
            print(f"ℹ Found {extreme_outliers} turnover rates above 200% (high churn schools)")
            print(f"  Max value: {rate_values.max():.1f}%")

        # Filter to count metrics
        count_df = kpi_df[kpi_df['metric'].isin(['teacher_turnover_count', 'teacher_count'])]
        count_values = count_df['value'].dropna()

        # Counts should be non-negative
        assert (count_values >= 0).all(), "Found negative teacher counts"

        print(f"✓ Turnover rate range: {rate_values.min():.1f}% - {rate_values.max():.1f}%")
